...
router.get('/connectedclients/:outletid', function (req, res, next) {
    var outletid = req.params.outletid;
	console.log("connectedclients Inside : " + outletid );
     if (socket != undefined)
    {
		console.log("connectedclients -- Inside Defined : " + outletid );
        socket.emit("get-connected-clients-from-server", function (receive_connected_clients) {
            for(var i=0; i< receive_connected_clients.length; i++) {
                if(receive_connected_clients[i]==outletid)
                {
                    res.send('Connected');
					console.log("connectedclients --connected: " + outletid );
                }
                else {
                    res.send('Not Connected');
					console.log("connectedclients -- Not connected receive: " + outletid );
                }
            }
        });
         res.send('Not Connected');
		 console.log("connectedclients -- Not connected: Not inside Loop " + outletid );
    }
    else {
        res.send('Not Connected');
		console.log("connectedclients -- Not connected: Undefined " + outletid );
    }
});
